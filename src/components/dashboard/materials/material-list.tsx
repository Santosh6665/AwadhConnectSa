...
import { MoreHorizontal, Edit, Trash2, Link as LinkIcon, FileText, CheckCircle } from 'lucide-react';
import { format } from 'date-fns';
import CompletionStatusDialog from './completion-status-dialog';
...
export default function MaterialList({ materials, onEdit, onDelete, isSaving }: MaterialListProps) {
  const [selectedMaterial, setSelectedMaterial] = React.useState<StudyMaterial | null>(null);
  const [isStatusOpen, setIsStatusOpen] = React.useState(false);

  const handleViewStatus = (item: StudyMaterial) => {
            ...
            {materials.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.subject}</TableCell>
                 <TableCell>
                    <Button variant="link" className="p-0 h-auto" onClick={() => handleViewStatus(item)}>
                       <CheckCircle className="mr-2 h-4 w-4 text-green-600" />
                       {item.completedBy?.length || 0} students
                    </Button>
                </TableCell>
